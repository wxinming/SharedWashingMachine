...
--		uartc代表美的滚筒投币+扫码
--		uart代表美的扫码
--		pul_uartc代表美的波轮投币+扫码
--		pul_uart代表美的波轮扫码
--		hili_uart代表海狸扫码
--		gpio代表所有支持脉冲设备,逻辑处理和算法自己实现
...
--获取网络模式
--@note   获取到的模式由_G.PROJECT决定
--@note	  4G_v2.0.3,net库中getnetmode改为getNetMode
--@param  无
--@return string:网络模式
function getNetMode()
	local mode,result = 0,""
	if _G.PROJECT == "AIR720" then
		mode = net.getNetMode()
		if mode == 0 then
			result = "noNet"
		elseif mode == 1 then
...
--@note   此处算法不要随意更改,官方固件不可使用带FLOAT版本
--				时	分	秒
